...
class Uji_Interst_Admin extends Uji_Interst_Admin_API{
	...
	public function __construct ( $file ) {
		...
		$this->opt_name = __( 'Interstitial Ads Options (Version 1.5)', 'ujinter' );
		...
		add_action( 'save_post', array( &$this, 'interads_save') );	
                 ...
                 //Remove
		add_action( 'admin_head', array( &$this, 'remove_inter_ad' ) );
                
                //Admin Notice
                add_action( 'admin_notices', array( &$this, 'free_options_ad' ) );
	}
	 ...
	 * @since  1.0
	 */
	public function remove_inter_ad( ){  
		 $screen = get_current_screen();
		$published_posts = wp_count_posts( 'interads' ); 
		 if  ( 'interads' == $screen->post_type ) {
			if( (int) $published_posts->publish > 0 && 'interads' == $screen->post_type ){
				remove_submenu_page( 'edit.php?post_type=interads', 'post-new.php?post_type=interads' );
				add_action('admin_footer', array( &$this, 'add_interads_css' ) );
			}
			
		 }
		 if ( $published_posts->publish >= 1 && 'popups' == $screen->post_type ) {
			 	add_action('admin_footer', array( &$this, 'add_interads_css_one' ) );
		 }
		 
		 if ( $published_posts->publish >= 2 && 'popups' != $screen->post_type ) {
			 	add_action('admin_footer', array( &$this, 'add_interads_css_two' ) );
		 }
	}
	...
	public function interads_prover( $post ) {
        ...
        public function free_options_ad() {
                $is_error = wp_count_posts('interads');
                
                if ($is_error->publish > 1):
            ?>
                    <div class="update-nag">
                        <p>The Free version support only one Ad. Please <a href="http://www.wpmanage.com/interstitial-ads"> Upgrade</a> for <strong>multiple Ads<strong> </p>
                    </div>
            <?php
                endif;
        }
	...
	public function interads_save( $post_id ) {
		...
		//if ( !isset($_POST['interads_edit_nonce']) || (isset($_POST['interads_edit_nonce']) && !wp_verify_nonce( $_POST['interads_edit_nonce'], 'interads_edit_nonce' ))) return $post_id;
		...
		// Save fields
			...
			if( isset($_POST['show_title'] ) ) update_post_meta($post_id, 'show_title', esc_html(stripslashes($_POST['show_title']))); else update_post_meta($post_id, 'show_title', '');
                        if( isset($_POST['ads_impressions'] ) ) update_post_meta($post_id, 'ads_impressions', esc_html(stripslashes($_POST['ads_impressions']))); else update_post_meta($post_id, 'ads_impressions', '');
			...
			if( isset($_POST['post_category'] ) ) update_post_meta($post_id, 'ad_post_category', $_POST['post_category'] );  else update_post_meta($post_id, 'ad_post_category', '');
			...
			if( isset($_POST['on_wait_time'] ) ) update_post_meta($post_id, 'on_wait_time', esc_html(stripslashes($_POST['on_wait_time']))); else update_post_meta($post_id, 'on_wait_time', '');
		}
